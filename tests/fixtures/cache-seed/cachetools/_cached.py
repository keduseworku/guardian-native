...
def _cache_invalidate(cache, key, lock):
    if cache is None:
        return lambda *args, **kwargs: False
    elif lock is None:

        def cache_invalidate(*args, **kwargs):
            return _invalidate(cache, key(*args, **kwargs))
...
def _wrapper(func, cache, key, lock=None, cond=None, info=None):
    ...
    cache_lock = lock if lock is not None else cond
    wrapper.cache = cache
    wrapper.cache_key = key
    wrapper.cache_lock = cache_lock
    wrapper.cache_condition = cond
    wrapper.cache_invalidate = _cache_invalidate(cache, key, cache_lock)
